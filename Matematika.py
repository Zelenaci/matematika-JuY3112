...
class Application(tk.Tk):
    ...
    def __init__(self):
        ...
        self.lblVysl.grid(row=1, column=4)
        self.generovani_prikladu()
        self.entry = Entry(self, width=3)
    ...
    def kontrola(self, event):
        try:
            if int(self.entry.get()) == self.vysledek:
                self.lblVysl.config(text=random.choice(["Výborně", "Správně", "Úžasné", "Velmi Dobře",":)"]))

            else:
                self.lblVysl.config(text=random.choice(["Špatně","To nebylo správně","Nesprávně",":("]))
